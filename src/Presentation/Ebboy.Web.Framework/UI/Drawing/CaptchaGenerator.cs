...
namespace Ebboy.Web.Framework.UI.Drawing
{
    public static class CaptchaGenerator
    {
        ...
        public static byte[] Generate(Action<string, string> processCode, string sessionName, int width, int height, int len, int fontsize, bool isbold, int charW, int pixelcount, int beziercount, Pen pen)
        {
            ...
            for (short i = 0; i <= code.Length - 1; i++)
            {
                ...
                g.DrawString(code.ToString().Substring(i, 1), font, mybrush, i * charW + 1, 0);//在矩形内画出字符串 
            }
        ...
        {
            return GenerateImage(processCode, sessionName, 45, 18, 4, 12, false, 10, 40, 0, pen);
        }
        ...
        /// <param name="pen">绘制边框的对象（表现),为null时不绘制</param>
        /// 创 建 者：Loamen.com
        public static byte[] GenerateImage(Action<string, string> processCode, string sessionName, int width, int height, int len, Pen pen)
        {
            return GenerateImage(processCode, sessionName, width, height, len, 12, false, 12, 40, 0, pen);
        }
        ...
        {
            return GenerateImage(processCode, sessionName, width, height, len, 12, false, charW, pixelcount, 0, pen);
        }
        ...
        {
            return GenerateImage(processCode, sessionName, width, height, len, fontsize, isbold, charW, 40, 0, pen);
        }
        ...
        {
            return GenerateImage(processCode, sessionName, width, height, len, fontsize, isbold, charW, pixelcount, 0, pen);
        }
